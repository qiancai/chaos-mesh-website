...
IOChaos 允许你模拟文件系统错误，如IO 延迟和读/写错误。 当你的程序运行 IO 系统调用时，它可能会注入延迟和故障，如 `打开`， `读取`和 `写入`。
...
假设你正在使用 `示例/io-mixedexample.yaml`，你可以运行以下命令来创建一个chaos实验：
...
- **延迟**: IO 延迟操作。 你可以在IO 操作返回结果之前指定延迟。
- **故障**: IO 故障操作。 在此模式下，IO 操作返回一个错误。
- **吸引覆盖**: 覆盖文件的属性。

### 延迟

如果你正在使用 `延迟` 动作，你可以编辑下面的规格：
...
> **注意**:
> 
> 此属性可以通过 Linux 内核缓存，所以如果你的程序以前曾经访问过它，它可能没什么作用。
